...
@RunWith(AndroidJUnit4.class) @LargeTest public class MainActivityTest {
  ...
  @Test public void showsSuperHeronNameOnDetailActivityToolbarWhenTappingOnARow() {
    List<SuperHero> superHeroes = givenThereAreSomeSuperHeroes(ANY_NUMBER_OF_SUPER_HEROES);

    startActivity();

    for (int i = 0; i < ANY_NUMBER_OF_SUPER_HEROES; i++) {
      onView(withId(R.id.recycler_view)).perform(RecyclerViewActions.actionOnItemAtPosition(i, click()));
      onToolbarWithTitle(superHeroes.get(i).getName()).check(matches(isDisplayed()));
      pressBack();
    }
  }
  ...
  @Test public void opensSuperHeroDetailActivityOnRecyclerViewItemTapped() {
    ...
    onView(withId(R.id.recycler_view)).perform(RecyclerViewActions.actionOnItemAtPosition(superHeroIndex, click()));
